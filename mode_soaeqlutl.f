...
! Calculate mole fraction (molecules only)
     ...
     &    / MW_WATER ![g/mol] ==> umol/m3
      ...
      DO I=1,NAAERO  !All molecules and ions
       TMOL(:) = TMOL(:)   
     &       + AERO_AQ(:,I)/MW_SOA(I)
      ENDDO
      TMOLINV(:)=1.d0/TMOL(:)
...
!(i.e. acids and their dissociating products)
      ...
      DO I=1,NBSP
       ...
       X_AQ(:,I)=AERO_AQ(:,COMP_IDX)/MW_SOA(COMP_IDX)*TMOLINV(:)
       COMP_IDX=COMP_IDX + 1
          
       !Add up any concentration of dissociated species (HA- or A2-)
       DO J=2,NK(I)
            X_AQ(:,I)=X_AQ(:,I)+AERO_AQ(:,COMP_IDX)/
     &           MW_SOA(COMP_IDX)*TMOLINV(:)
       COMP_IDX=COMP_IDX + 1
       ENDDO
      ENDDO
...
! according to the standard state of Raoults Law 
      ...
      DO I=1,NBSP
       ...
       X_AQ(:,I) =  1.d-13 !fxm: put this back after testingTMOL_AQ(:) * TINY

       !Make sure sum(X)=1 after modifying one value
       !ddw: NMOL_AQ is 18 here and the 18th molecule is water
       DO J=1,NMOL_AQ
...
!GAS_MEAN(:,:)=0.5d0*(GAS_AQ(:,:) + GAS_ORG(:,:))
      ...
      IF(LPRINT)THEN
       DO I=1, NBSP
          write(6,*)"GAS_MEAN, GAS_AQ, GAS_ORG", I, 
     &        GAS_MEAN(:,I), GAS_AQ(:,I), GAS_ORG(:,I)
       ENDDO
      ENDIF
...
!INPUT
      REAL, DIMENSION(:,:), INTENT(IN)   :: totAER_AQ   ![ug/m3] total (H2A+HA-+A(2-)) aq aerosol
      REAL, DIMENSION(:,:), INTENT(IN)   :: NOMINATOR   ![-] factor which says how much of totAER_AQ which
                                                      !has dissociated
      REAL, DIMENSION(:,:), INTENT(IN)   :: K           ![m3/ug and mole/kg] Henry and dissociation constants
...
!LOCAL
     ...
     &          * MW_SOA(COMP_IDX)/MW_SOA(COMP_IDX-1)  !eqn 6/7 in Griffin 2003
