...
!Written by Alf Grini, alf.grini@cnrm.meteo.fr, 2005
     ...
     &  ,GAMMA_AQ_RAOULT, GAMMA_AQ_HENRY   ! ddw --- output the
coefficient 
     &)
...
!Local
      ...
      INTEGER, PARAMETER                      :: MAX_ITER=6                    !Number of iterations performed (default is 6)
...
!       ENDIF
     ...
     &       )
       ...
       !Adjust mass balance for all components, 
       !both in aq phase and in organic phase
       CALL MB_ADJUST(              
...
!       ENDIF
     ...
     &       )
      ...
      DEALLOCATE(GAMMA_AQ)
!ddw      DEALLOCATE(GAMMA_AQ_RAOULT)
!ddw      DEALLOCATE(GAMMA_AQ_HENRY)
