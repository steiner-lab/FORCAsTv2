...
!Allocate memory
      ALLOCATE(AEROMOLEC(SIZE(AERO,1),NBSP))
...
!Start code
      ...
      ELSE  !        zsrflag = 1 
       ...
       COMP_IDX=1
       DO I =1,NBSP
          AEROMOLEC(:,I)=AERO(:,COMP_IDX)/MW_SOA(COMP_IDX)
          COMP_IDX=COMP_IDX+1
          
          DO J=2,NK(I)
             AEROMOLEC(:,I)=AEROMOLEC(:,I) 
     &             + AERO(:,COMP_IDX)/MW_SOA(COMP_IDX)
       ...
       DO I=1,NBSP
          ...
          DO IX=1,SIZE(AERO,1) !Loop on points
             ...
             !==> transforms lower RH into a 1-11 scale, we know binsolu for 0, 1, 2, 3, ..10
             IRH1(IX)=int(RH(IX)/RHGRAD) +1 
          
             !==> transforms upper RH into a 1-11 scale, we know binsolu for 0, 1, 2, 3, ..10
             IRH2(IX)=int(RH(IX)/RHGRAD) +2 
