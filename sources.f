...
!ka - replaced above line with soil NO emission rate read in from inputn
       source(1,lno)= (efno/0.05)*0.325e-12/dzf(1)*ft    ! default
c      source(1,lno)= (efno/0.05)*0.325e-12/dzf(1)*ft*30.0 
...
c#################################################################
      ...
      subroutine sinks(vcp,akh,lprin,tstamp,ustarb)
...
c which is the ratio of the diffusivity of H2O to that of the gas.
c The default values are 1e.-22, 0.0 and 1.0
c ------------------- rbl:molecular diffusion resistance --------
c ------------------- rmeso:mesophyll resistance --------
c ------------------- rcut:cuticular resistance --------
c ------------ rst:stomatal resistance (for water vapor???) ----
c####################################################################
c
...
c -----------------------OLD parameterization ----------- ----
        if (OBSavailability(igas).eq.0.0) then ! ddw (2020 Mar) 
     ...
     &           2./(rbl+rcut)
c ddw-- the deposition velocity suggested by Karl et al. (2010) 
            if (igas.eq.275.or.igas.eq.364) then
               cond = 6.86*cond
            end if
            condf=condf+frarea(i,j)*cond ! deposition velocity
            enddo
c ---------------- ddw dry deposition modification based on Nguyen 2015-----------
        else  
            Raero = 10.d0  !aerodynamic resistance (small compared to other resistances)
            Rbl = visco/(DH2O/dfakt(igas))/ustarb*((100.d0*leafthick
     &            *ustarb/TLAI**2/visco))**(1./3.)
            rmeso=(henry(igas)/(50.d0*gascst*THETA(j))+100.d0*f0(igas))
     &            **(-1) 
            rcut = (1.0E-4*henry(igas)/(gascst*THETA(j))+f0(igas))**(-1)
            condf=0.0
            do i=1,itotp1
            rstij = min(3000.d0, rst(i,j))
            Rc = (1.d0/(rstij*dfakt(igas)+rmeso)+1.d0/rcut)**(-1) 
            cond=1./(Raero+Rbl+Rc)
c            if(igas.ge.105.and.igas.le.111.or.
c     & igas.eq.115.or.igas.eq.317) then
c               cond = cond*0.25
c            endif
            condf=condf+frarea(i,j)*cond ! deposition velocity for all leaf angles in layer j
            enddo
        end if
c       checking the dep velocity
c        if(igas.eq.276.or.igas.eq.223.or.igas.eq.103.or.igas.eq.325.or.
c     & igas.eq.280) condf = 0.0 
c ------------------------------------------------------------------
